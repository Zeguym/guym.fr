...
# Diferents types de tests d'architecture

Voici quelques types courants de tests d'architecture que vous pouvez implémenter dans vos projets .NET :
...
- Les classes de la couche **Domain** doivent être internes pour éviter qu'elles soient utilisées en dehors du projet.
...
{
    [Fact]
    public void HighLevelModules_Should_Depend_On_Abstractions()
    {
        var result = Types.InAssembly(typeof(MyHighLevelClass).Assembly)
            .That().ResideInNamespace("MyProject.Application")
            .Should().HaveDependencyOn("MyProject.Domain.Abstractions")
            .GetResult();

        Assert.True(result.IsSuccessful, "High-level modules should depend on abstractions, not implementations.");
    }
}
```
