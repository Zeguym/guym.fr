...
- **Analyser le code** avec des outils comme `dotnet format` ou `dotnet analyzers`.
...
## 3. Ajouter un hook pour exécuter les tests

Ajoutez un hook `pre-commit` ou `pre-push` pour exécuter les tests avant un commit ou un push :
...
- Avant chaque commit, Husky exécute la commande `dotnet test`.
...
### Ajouter un hook `pre-push` pour exécuter les tests avant un push :
